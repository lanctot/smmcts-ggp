...
\title{Monte Carlo Tree Search Variants for General Simultaneous Move Games}
...
The extension to simultaneous move games, where all player choose actions simultaneously each turn, 
is non-trivial due to the complexity of this class of games. In this paper, we describe simultaneous move MCTS 
and analyze its application in general, over nine different simultaneous move games. We use several possible 
variants, ones that include both deterministic and stochastic selection strategies as well as different game modes, 
and characterize the practical performance of each one. Our results indicate that the relative performance of 
each variant depends strongly on the game and the opponent, and that parameter tuning can also not be as 
straight-forward as the purely sequential case. Overall, Decoupled UCT performs best in these games, despite its 
...
simultaneous move MCTS variants. First, we describe a general simultaneous move MCTS framework along with several variants
...
the empirical results and discuss our findings. Finally, in Section~\ref{sec:conc} we conclude. 
...
the game. Four nodes are shown, and at each node, each player has two individual moves (for the row player: top, bottom) and 
(column player: left, right), leading to four joint actions per node.
\label{fig:example}}
\end{figure}
...
Two-player constant-sum simultaneous move games can be solved (\ie Nash equilibrium strategies can be computed) using backward 
induction~\cite{Ross71Goofspiel,Buro03OshiZumo,Rhoads12Computer}. The main idea is to start from the 
endgame positions and individually compute the values of the subgames and work back to the root state. 
For the example in Figure 1, the value of the game on the bottom-left is 2 because two strategies are strictly dominated, and the 
value of Matching Pennies is 0, and so these perfect values represent the correct game-theoretic values in the parent nodes. 
In MCTS, these values will be approximated using Monte Carlo sampling, and the variants will make use of these estimates when selecting moves.
As in purely sequential games, search techniques aim to approximate these underlying game-theoretic algorithms, but in simultaneous 
move games the connection is not straight-forward. 
...
In Simultaneous Move MCTS (SM-MCTS), the main difference is that during the selection step, at each node, a {\it joint} 
action is selected. The convergence to an 
...
We focus on a number of SM-MCTS variants. The most popular one is straight-forward adaption of UCT to the 
simultaneous move case. Though it has been shown to not converge to a Nash equilibrium even in a one-shot single-state 
game~\cite{Shafiei09}, it has nonetheless been popular and successful in general game playing. 
The second one is Exp3, first introduced as an adversarial bandit algorithm that can be applied to learning in 
...
move MCTS, the simulation would consist of two joint actions, $(t,l) \rightarrow (t,l) \rightarrow 2$. However, 
...
will learn to play the move that has the least chances of being penalized by an opponent. In fact, in the classical 
...
done in classical $\alpha\beta$ search in abstract combat games~\cite{Kovarsky05RAB}.
...
Tron              & $C=1.8$ & $C=2.0$ & $\gamma = 0.3$ & $\gamma = 0.30$ \\
 \hline
\end{tabular}
\caption{Parameters values. \label{table:parameters}}
\end{table}


Before the different algorithms are compared, their parameters are tuned by self play where different parameter values are compared against a reference parameter. In DUCT, and SUCT the default parameter is $C=1.4$. The reference parameters for Exp3 and RM are set to $\gamma = 0.2$ and $\gamma = 0.025$, respectively, because they are the optimal parameters in Goofspiel \cite{Lanctot13Goofspiel}. The tuned parameter values are shown in Table \ref{table:parameters}. 
...
This kind of behavior also holds for the other algorithms. Therefore, this figure supports choosing a separate parameter value for each game and algorithm.
...
Tables \ref{tbl:vsduct}, \ref{tbl:summary}, and \ref{tbl:cross} show our performance results for each variant in the games we have tried. 
Tables report win percentage with 95\% confidence intervals. Draws counts as a half win and half loss. 

Table \ref{tbl:vsduct} shows the win rate of the algorithms against DUCT. This table is important, because DUCT is the standard benchmark often used
in the GGP competitions. Therefore, it is interesting to investigate whether the alternatives are preferred. Based on the results in Table \ref{tbl:vsduct}
it seems that DUCT indeed performs well in almost most games. This is consistent with the existing successes in general game playing. 

However, in Goofspiel and Oshi-Zumo, Regret Matching performs significantly better than DUCT. Exp3 also outperforms DUCT in Goofspiel. 
...
solved~\cite{Buro03OshiZumo,Rhoads12Computer} and their optimal strategies are mixed distributions. In Goofspiel, the importance of mixing
has been justified previously, any deterministic strategy has a best response that exploit it by a lot. So, these stochastic selection strategies, 
...
We also notice that SUCT outperforms DUCT in Tron. This is unsurprising, as we have discovered in previous work that mistakes in Tron are
unforgiving~\cite{Lanctot13Tron}. Because SUCT plays defensively avoiding mistakes that could lead to opponent best responses, this could 
have a positive effect in a game like Tron. DUCT does seem to filter out clearly bad moves in practice, but since the estimates are based on 
joint selections, it is possible that the opponent does not ``cooperate'' in helping identify the bad moves which are then not filtered out 
as effectively as in SUCT where the opponent is given knowledge of the acting player's move. 
...
well in Tron~\cite{Perick12Comparison,Lanctot13Tron}. However, we were unable to reproduce these gains in this general context when using random
playout policies. In our case, DUCT and DUCB1T seemed on to be equally strong.  
...
However, SUCT beats RM in Goofspiel. We suspect the reason for this surprising result is that the algorithms in simultaneous move games are 
very sensitive to the parameters. As these parameters are tuned in self play, it could be that we were overfitting the parameters, because 
...
against both DUCT and SUCT in most games. Overall, if these nine games form a good representation of the type of games to be played, and there 
is no domain knowledge, then DUCT seems to be a particularly safe choice, winning 66.56\% of games overall. However, Sequential UCT winning 59.79\% 
of games, is not too far behind, can often be easier to implement, and does seem to do fairly well against Regret Matching in games where mixing
is important such as Goofspiel.  
...
of a real-time game. For example, SM-MCTS could be used as a high-level multistage planner in real-time strategy games by treating the action of each agent
