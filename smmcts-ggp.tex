...
deterministic and stochastic selection strategies and we characterize the practical performance of each one. 
...
first used in general game-playing~\cite{Finnsson08}, is a variant we call {\it Decoupled UCT} (DUCT), where each player uses UCB to select 
...
the empirical results and discuss them further. Finally, in Section~\ref{sec:conc} we conclude. 
...
(column player: left, right), leading to four joint moves per node.
\label{fig:example}}
\end{figure}
...
For example, in Matching Pennies, each player has two moves (heads or tails). The row player receives a payoff of 1 if both players choose the same move and 0 if they do not match. 
...
value of Matching Pennies is $0.5$. These values represent the correct game-theoretic value for a joint move in the parent node.
In MCTS, these values are approximated using Monte Carlo sampling, and the variants will the estimates when selecting moves.
...
game, it has nonetheless been popular and successful in general game playing. 
...
and move sets $\cA_{\Max}(s) = \{t,b\}, \cA_{\Min}(s) = \{l,r\}$ representing top, bottom, left, and right. 
...
{\it weighted} reward, where each value gets scaled by the the probability of it having been sampled. 
