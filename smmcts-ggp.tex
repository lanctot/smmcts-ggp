...
variants, ones that include both determinstic and stochastic selection strategies as well as different game modes, 
...
\end{subfigure}
\caption{An example of a two-player simultaneous game which has Matching Pennies as a subgame.
...
games, was also applied successfully to the popular online card game Urban Rivals~\cite{Teytaud11Upper,StPierre12Online}. 
...
Finally, we also try Sequential UCT, which was previously used in Tron~\cite{Samothrakis10Tron,Lanctot13Tron}.
...
This kind of behaviour also holds for the other algorithms. Therefore, this figure supports choosing a separate parameter value for each game and algorithm.
...
has been justified previous, any deterministic strategy has a best response that exploit it by a lot. So, these stochastic selection strategies, 
...
have a positive effect in a game like Tron. What we have noticed specifically in Tron is that the results are significantly different when 
...
well in Tron~\cite{Perick12Comparison,Lanctot13Tron}. However, we were unable to reproduce these gains in this context when using random
playout policies. 
...
However, SUCT beats RM in Goofspiel. We think that the reason for this surprising result is that the algorithms in simultaneous move games are 
...
the optimal parameter value for a certain algorithm also depends on the algorithm used by the opponent. But also, it could also be that one 
variant is acting as a sort of best response to the other. 
...
In this paper, we present simultaneous move MCTS and several variants, evaluating their performance in nine games used in the general game playing. 
We investigate several variants: DUCT, SUCT, Exp3 and RM. This work entails two important conclusions. First, 

For future work, we hope to try games and variants with more than two players, for example as a strategic reasoner module in the seven-player 
simultanoues move game Diplomacy~\cite{Fabregues11DipGame}. 
